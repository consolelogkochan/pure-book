  ...
  return (
    <form key={selectedBooking.id} onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {/* お客様情報（readOnly — fieldset 外に配置し、送信中のグレーアウトを防ぐ） */}
        ...
        {/* 編集可能なフィールドを fieldset で囲む（アンケート表示は form 要素なので disabled の影響なし） */}
        <fieldset disabled={isSubmitting} className="border-0 p-0 m-0 space-y-6">
          ...
          {/* アンケート結果（表示のみ — form 要素なし、disabled の視覚影響なし） */}
